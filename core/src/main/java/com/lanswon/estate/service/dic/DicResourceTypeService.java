...
import com.lanswon.estate.mapper.DicResourceTypeMapper;
...
@Service
@Slf4j
public class DicResourceTypeService {

	@Resource
	private DicResourceTypeMapper dicResourceTypeMapper;


	public DTO insertResourceType(DicResourceType ResourceType) {
		log.info("新增共有类型数据");
		if (dicResourceTypeMapper.insert(ResourceType) == 0){
	...
	public DTO deleteResourceType(String id) {
		log.info("删除id为-->{}的共有情况数据",id);
		// 删除 共有情况
		if (dicResourceTypeMapper.deleteById(id) == 0){
	...
	public DTO updateResourceType(DicResourceType ResourceType) {
		log.info("更新共有为-->{}的共有情况,",ResourceType.getId());

		if (dicResourceTypeMapper.updateById(ResourceType) == 0){
			log.error(CustomRtnEnum.ERROR_BAD_SQL.toString());
			return new SimpleRtnDTO(CustomRtnEnum.ERROR_BAD_SQL.getStatus(),CustomRtnEnum.ERROR_BAD_SQL.getMsg());
		}
		log.info(CustomRtnEnum.SUCCESS.toString());
	...
	public DTO getAllResourceTypeInfo(Page<DicResourceType> page,int asc) {
		log.info("获得所有共有情况信息");

		return getAllDataWithPage(page,asc, dicResourceTypeMapper);
	}


	public DTO getAllResourceTypeWithoutPage() {
		log.info("获得所有共有情况信息 -- 不分页");
		return getAllDataWithoutPage(dicResourceTypeMapper);
	}
