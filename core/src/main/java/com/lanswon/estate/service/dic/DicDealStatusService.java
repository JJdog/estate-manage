...
import com.lanswon.estate.mapper.DicDealStatusMapper;
...
@Service
@Slf4j
public class DicDealStatusService {

	@Resource
	private DicDealStatusMapper dicDealStatusMapper;


	/**
	 * 新增共有类型情况
	 * @param dealStatus 共有类型
	 * @return dto
	 */
	public DTO insertDealStatus(DicDealStatus dealStatus) {
		log.info("新增共有类型数据");
		if (dicDealStatusMapper.insert(dealStatus) == 0){
	...
	public DTO deleteDealStatus(String id) {
		log.info("删除id为-->{}的共有情况数据",id);
		// 删除 共有情况
		if (dicDealStatusMapper.deleteById(id) == 0){
	...
	/**
	 * 更新共有情况
	 * @param dealStatus 共有情况
	 * @return dto
	 */
	public DTO updateDealStatus(DicDealStatus dealStatus) {
		log.info("更新共有为-->{}的共有情况,",dealStatus.getId());

		if (dicDealStatusMapper.updateById(dealStatus) == 0){
			log.error(CustomRtnEnum.ERROR_BAD_SQL.toString());
			return new SimpleRtnDTO(CustomRtnEnum.ERROR_BAD_SQL.getStatus(),CustomRtnEnum.ERROR_BAD_SQL.getMsg());
		}
		log.info(CustomRtnEnum.SUCCESS.toString());
	 ...
	 * @return dto
	 */
	public DTO getAllDealStatusInfo(Page<DicDealStatus> page, int asc) {
		log.info("获得所有共有情况信息");

		return getAllDataWithPage(page,asc,dicDealStatusMapper);
	}
	 ...
	 * @return dto
	 */
	public DTO getAllDealStatusWithoutPage() {
		log.info("获得所有共有情况信息");

		return getAllDataWithoutPage(dicDealStatusMapper);
	}
