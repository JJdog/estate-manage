...
@Service
@Slf4j
public class DicAssetsCoService {
	 ...
	 * @return dto
	 */
	public DTO insertAssetsCo(DicAssetsCo co) {
		log.info("新增共有类型数据");
		if (dicAssetsCoMapper.insert(co) == 0){
	...
	public DTO deleteAssetsCo(String id) {
		log.info("删除id为-->{}的共有情况数据",id);
		// 删除 共有情况
		if (dicAssetsCoMapper.deleteById(id) == 0){
	 ...
	 * @return dto
	 */
	public DTO updateAssetsCo(DicAssetsCo co) {
		log.info("更新共有为-->{}的共有情况,",co.getId());

		if (dicAssetsCoMapper.updateById(co) == 0){
			log.error(CustomRtnEnum.ERROR_BAD_SQL.toString());
			return new SimpleRtnDTO(CustomRtnEnum.ERROR_BAD_SQL.getStatus(),CustomRtnEnum.ERROR_BAD_SQL.getMsg());
		}
		log.info(CustomRtnEnum.SUCCESS.toString());
	 ...
	 * @return dto
	 */
	public DTO getAllAssetsCoInfo(Page<DicAssetsCo> page, int asc) {
		log.info("获得所有共有情况信息");

		return getAllDataWithPage(page,asc,dicAssetsCoMapper);
	}
	 ...
	 * @return dto
	 */
	public DTO getAllAssetsCoWithoutPage() {
		log.info("获得所有共有情况信息");

		return getAllDataWithoutPage(dicAssetsCoMapper);
	}
