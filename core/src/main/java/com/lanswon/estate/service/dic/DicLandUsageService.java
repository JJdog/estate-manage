...
import com.lanswon.estate.mapper.DicLandUsageMapper;
...
@Service
@Slf4j
public class DicLandUsageService {

	@Resource
	private DicLandUsageMapper dicLandUsageMapper;


	public DTO insertLandUsage(DicLandUsage landUsage) {
		log.info("新增共有类型数据");
		if (dicLandUsageMapper.insert(landUsage) == 0){
	...
	public DTO deleteLandUsage(String id) {
		log.info("删除id为-->{}的共有情况数据",id);
		// 删除 共有情况
		if (dicLandUsageMapper.deleteById(id) == 0){
	...
	public DTO updateLandUsage(DicLandUsage landUsage) {
		log.info("更新共有为-->{}的共有情况,",landUsage.getId());

		if (dicLandUsageMapper.updateById(landUsage) == 0){
			log.error(CustomRtnEnum.ERROR_BAD_SQL.toString());
			return new SimpleRtnDTO(CustomRtnEnum.ERROR_BAD_SQL.getStatus(),CustomRtnEnum.ERROR_BAD_SQL.getMsg());
		}
		log.info(CustomRtnEnum.SUCCESS.toString());
	...
	public DTO getAllLandUsageInfo(Page<DicLandUsage> page,int asc) {
		log.info("获得所有共有情况信息");

		return getAllDataWithPage(page,asc, dicLandUsageMapper);
	}


	public DTO getAllLandUsageWithoutPage() {
		log.info("获得所有共有情况信息 -- 不分页");
		return getAllDataWithoutPage(dicLandUsageMapper);
	}
