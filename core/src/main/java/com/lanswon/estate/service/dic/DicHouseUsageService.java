...
import com.lanswon.estate.mapper.DicHouseUsageMapper;
...
@Service
@Slf4j
public class DicHouseUsageService {


	@Resource
	private DicHouseUsageMapper dicHouseUsageMapper;

	public DTO insertHouseUsage(DicHouseUsage usage) {
		log.info("新增共有类型数据");
		if (dicHouseUsageMapper.insert(usage) == 0){
	...
	public DTO deleteHouseUsage(String id) {
		log.info("删除id为-->{}的共有情况数据",id);
		// 删除 共有情况
		if (dicHouseUsageMapper.deleteById(id) == 0){
	...
	public DTO updateHouseUsage(DicHouseUsage usage) {
		log.info("更新共有为-->{}的共有情况,",usage.getId());

		if (dicHouseUsageMapper.updateById(usage) == 0){
			log.error(CustomRtnEnum.ERROR_BAD_SQL.toString());
			return new SimpleRtnDTO(CustomRtnEnum.ERROR_BAD_SQL.getStatus(),CustomRtnEnum.ERROR_BAD_SQL.getMsg());
		}
		log.info(CustomRtnEnum.SUCCESS.toString());
	...
	public DTO getAllHouseUsageInfo(Page<DicHouseUsage> page,int asc) {
		log.info("获得所有共有情况信息");

		return getAllDataWithPage(page,asc, dicHouseUsageMapper);
	}


	public DTO getAllHouseUsageWithoutPage() {
		log.info("获得所有共有情况信息 -- 不分页");
		return getAllDataWithoutPage(dicHouseUsageMapper);
	}
